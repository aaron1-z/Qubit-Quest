...
class StartScene extends Phaser.Scene {
  ...
  preload() {}
  create() {
    const width = (this.scale && this.scale.width) || window.innerWidth || 1024;
    const height = (this.scale && this.scale.height) || window.innerHeight || 640;

    this.cameras.main.setAlpha(1);
    this.cameras.main.setBackgroundColor("#05020a");
    this.cameras.main.fadeIn(600, 0, 0, 0);

    this._glow = this.add.graphics();
    this._glowPhase = 0;
    this._glowTimer = this.time.addEvent({
      delay: 16, loop: true, callback: () => {
        this._glowPhase += 0.04;
        this._glow.clear();
        const alpha = 0.15 + Math.sin(this._glowPhase) * 0.08;
        this._glow.fillStyle(0x00ffff, alpha);
        this._glow.fillCircle(Math.round(width / 2), Math.round(height / 2), 180 + Math.sin(this._glowPhase * 2) * 10);
      }
    });

    this.add.text(width / 2, height / 2 - 120, "QUBIT QUEST", { fontSize: "48px", color: "#9ff", fontFamily: "monospace", fontStyle: "bold" }).setOrigin(0.5);
    this.add.text(width / 2, height / 2 - 60, "A Quantum Field Simulation Puzzle", { fontSize: "18px", color: "#bdf" }).setOrigin(0.5);

    const startBtn = this.add.text(width / 2, height / 2 + 60, "▶ Start Simulation", { fontSize: "24px", color: "#000", backgroundColor: "#9ff", padding: { x: 20, y: 10 } })
    ...
    startBtn.once("pointerdown", () => {
      try { this.playTone(440, 0.18, 0.08); } catch (e) {}
      this.cameras.main.fadeOut(300, 0, 0, 0);
      this.cameras.main.once("camerafadeoutcomplete", () => {
        if (this.scene.isActive("StartScene")) this.scene.stop("StartScene");
        this.scene.start("FinalScene", { skipTutorial: false });
      });
    });
  ...
  shutdown() {
    try {
      if (this._glowTimer) { this._glowTimer.remove(false); this._glowTimer = null; }
      if (this._glow && this._glow.destroy) { this._glow.destroy(); this._glow = null; }
      this.input.removeAllListeners();
      if (this._startBtn) { try { this._startBtn.destroy(); } catch (e) {} }
    } catch (e) {}
  }
}
...
class FinalScene extends Phaser.Scene {
  ...
  create(data = {}) {
    ...
    const resumeFn = () => {
      ...
      this.input.off('pointerdown', resumeFn);
      this.input.keyboard.off('keydown', resumeFn);
    };
    this.input.once('pointerdown', resumeFn);
    this.input.keyboard.once('keydown', resumeFn);

    const minWidth = this.boardW + this.panelW + 80;
    const w = Math.max(minWidth, Math.min(window.innerWidth || 1100, 1400));
    ...
    this.tiles = Array.from({ length: this.gridW }, () => []);
    this.createGrid();
    ...
    this.input.off('pointerdown', this._onPointer);
    this._onPointer = (ptr) => {
    ...
    this.input.on('pointerdown', this._onPointer);
    ...
    if (!this.skipTutorial) this.showInteractiveTutorial();
  ...
  createBackground() {
    try {
      ...
      const h = Math.max(1, this.canvasH);
      ...
      for (let y = 0; y < h; y += 6) {
        sc.fillStyle = (y % 12 === 0) ? 'rgba(255,255,255,0.01)' : 'rgba(255,255,255,0.004)';
        sc.fillRect(0, y, w, 1);
      }
      if (this.textures.exists('scan')) this.textures.remove('scan');
      this.textures.addCanvas('scan', scan);
      this.add.image(w / 2, h / 2, 'scan').setDepth(-5).setAlpha(0.03);
    } catch (e) {
      this.add.rectangle(this.scale.width / 2, this.canvasH / 2, this.scale.width, this.canvasH, 0x05020a).setDepth(-10);
    }
  }

  createWaveCanvas() {
    try {
      const w = Math.max(1, this.boardW);
      const h = Math.max(1, this.canvasH);
      if (this.textures.exists('wave')) this.textures.remove('wave');
      this.wave = this.textures.createCanvas('wave', w, h);
      if (!this.wave || !this.wave.source || !this.wave.source[0]) throw new Error('wave texture not available');
  ...
  createGrid() {
    for (let r = 0; r < this.gridW; r++) {
      ...
      for (let c = 0; c < this.gridW; c++) {
        ...
        const tile = this.add.rectangle(x, y, this.tileSize - 6, this.tileSize - 6, 0x12384f)
          .setStrokeStyle(3, 0x07131e).setDepth(1)
          .setInteractive();
        tile._r = r; tile._c = c;
        this.tiles[r][c] = tile;
  ...
  setTileType(r, c, type) {
    if (!this.tileMeta[`${r}_${c}`]) this.tileMeta[`${r}_${c}`] = { type: 'normal', linked: null, markers: [] };
    this.tileMeta[`${r}_${c}`].type = type;
    const tile = this.tiles?.[r]?.[c];
    if (!tile) return;
    const strokes = {
      photonGate: 0xffdd66, decoField: 0xff6666, portal: 0x66ffff,
      power: 0x33ff99, trap: 0xff4466, phase: 0xffcc00,
      hadamardBoost: 0x66ffcc, entangler: 0xaa66ff
    };
    tile.setStrokeStyle(4, strokes[type] || 0x07131e);
  }

  linkPortal(r1, c1, r2, c2) {
    if (!this.tileMeta[`${r1}_${c1}`]) this.tileMeta[`${r1}_${c1}`] = { type: 'normal', linked: null, markers: [] };
    if (!this.tileMeta[`${r2}_${c2}`]) this.tileMeta[`${r2}_${c2}`] = { type: 'normal', linked: null, markers: [] };
  ...
  createTrails() {
    try {
      this.add.particles('px', { frame: null, follow: this.playerA, lifespan: 420, scale: { start: 0.35, end: 0 }, tint: 0x111111, frequency: 50 });
      this.add.particles('px', { frame: null, follow: this.playerB, lifespan: 420, scale: { start: 0.35, end: 0 }, tint: 0xff66cc, frequency: 50 });
    } catch (e) {}
  }
  ...
  createUI() {
    ...
    for (let i = 0; i < this.patternLen; i++) {
      ...
      const label = this.add.text(rect.x - 8, rect.y - 8, "", { fontSize: "14px", color: "#9ff", fontStyle: "bold" }).setDepth(11);
      label.setOrigin(0, 0);
      this.patternIcons.push(rect);
      this.patternLabels.push(label);
    }
    ...
    this.endTurnBtn = this.add.text(px + 12, 520, "End Turn", { fontSize: "16px", color: "#000", backgroundColor: "#9ff", padding: { x: 8, y: 6 } })
  ...
  createLogOverlay() {
    ...
    this.input.keyboard.on('keydown-L', () => this.showLogOverlay(!this.logOpen, 220));
  }
  ...
  showInteractiveTutorial() {
    const overlay = this.add.rectangle(this.boardX + this.boardW / 2, this.canvasH / 2, this.boardW, this.canvasH, 0x000000, 0.85).setDepth(500);
    const lines = [
    ...
    const typeLine = () => {
      if (idx >= lines.length) {
        const clickText = this.add.text(this.boardX + this.boardW / 2, this.canvasH - 80, "▼ Click to begin ▼", { fontSize: "18px", color: "#9ff" }).setOrigin(0.5).setDepth(501);
        this.tweens.add({ targets: clickText, alpha: { from: 0.6, to: 1 }, duration: 800, yoyo: true, repeat: -1 });
        overlay.setInteractive().once('pointerdown', () => {
          overlay.disableInteractive();
          overlay.destroy();
          textObjs.forEach(t => t.destroy());
          clickText.destroy();
          this.playTone(640, 0.1, 0.07);
          this.log("Tutorial closed. Manipulate the field wisely!");
          this.input.keyboard.resetKeys();
        });
        return;
      }
      const text = this.add.text(this.boardX + this.boardW / 2, 120 + idx * 28, "", { fontSize: "18px", color: "#ccf", fontFamily: "monospace", align: "center" }).setDepth(501).setOrigin(0.5);
      textObjs.push(text);
      let charI = 0;
      ...
      const writeChar = () => {
        if (charI < line.length) {
          text.setText(line.substring(0, charI + 1));
          try { this.playTone(220 + Math.random() * 400, 0.02, 0.01); } catch (e) {}
          charI++;
          this.time.delayedCall(charDelay, writeChar);
        } else {
          idx++;
          this.time.delayedCall(200, typeLine);
        }
      };
      writeChar();
    };
    this.tweens.add({ targets: overlay, alpha: { from: 0.6, to: 0.85 }, duration: 1000, yoyo: true, repeat: -1, ease: 'Sine.easeInOut' });
    typeLine();
  }

  onTileClick(r, c, ptr) {
    const shift = !!(ptr && ptr.event && ptr.event.shiftKey);
    if (shift) {
      if (!this.entangleSelection) {
        this.entangleSelection = { r, c };
        if (this.tiles[r] && this.tiles[r][c]) this.tiles[r][c].setStrokeStyle(5, 0xffff66);
        ...
        if (a.r === r && a.c === c) {
          if (this.tiles[a.r] && this.tiles[a.r][a.c]) this.tiles[a.r][a.c].setStrokeStyle(3, 0x07131e);
          this.entangleSelection = null;
    ...
    this.time.delayedCall(800, () => { if (this.selector && this.selector.destroy) this.selector.destroy(); });
  }
  ...
  tryEntangle(r1, c1, r2, c2) {
    ...
    if (this.tiles[r1] && this.tiles[r1][c1]) this.tiles[r1][c1].setStrokeStyle(4, 0xaa66ff);
    if (this.tiles[r2] && this.tiles[r2][c2]) this.tiles[r2][c2].setStrokeStyle(4, 0xaa66ff);
    ...
    this.time.delayedCall(1100, () => { if (beam && beam.destroy) beam.destroy(); });
  ...
  async applyPhase() {
    ...
    if (this.energy < 1) { this.log("Need 1 energy for Phase operation."); this.recordAction("Q-fail", "energy"); return; }
  ...
  async applyLocalHadamard() {
    if (this._ending) return;
    if (this.energy < 2) { this.log("Need 2 energy for local Hadamard."); this.recordAction("H-fail", "energy"); return; }
    ...
    if (Math.random() > 0.2) { this.coherence = Math.min(100, this.coherence + 4); this.log(`Local Hadamard at column ${c} increased coherence.`); }
    else { this.coherence = Math.max(0, this.coherence - 5); this.log(`Hadamard misalignment caused coherence loss.`); }
  ...
  async peek() {
    ...
    this.energy -= 1;
    this.coherence = Math.max(0, this.coherence - 1.5);

    let res = null;
    try {
      res = await runQWalk({ n_positions: this.gridW, steps: 1, coin: 'hadamard', start_pos: this.currentStartPos, custom_coin_angles: [] });
    } catch (e) { res = null; }
    ...
    if (this.tiles[this.row] && this.tiles[this.row][b]) this.tiles[this.row][b].setStrokeStyle(5, 0xffff00);
    this.time.delayedCall(700, () => { if (this.tiles[this.row] && this.tiles[this.row][b]) this.tiles[this.row][b].setStrokeStyle(3, 0x07131e); });
  ...
  async measure() {
    if (this._ending) return;
    if (this.energy <= 0) { this.flashEnergyWarning(); return; }
    this.energy -= 1;
    ...
    this.updateUI();
    if (this.coherence <= 0 || this.lives <= 0) { this.endGame(false); return; }
    if (this.score >= 300) { this.endGame(true); return; }
    this.actionUsedThisTurn = true;
    this.autoPeekLog();
    if (this.turnBased) this.endTurnIfActionUsed();
  }
  ...
  addMarker(r, c, symbol, color) {
    this.markers = this.markers.filter(m => {
      if (m.r === r && m.c === c) {
        try { if (m.display && m.display.destroy) m.display.destroy(); } catch (e) {}
        return false;
      }
      return true;
    });
    ...
    const txt = this.add.text(mx, my, symbol, { fontSize: "26px", color: Phaser.Display.Color.IntegerToColor(color).rgba }).setOrigin(0.5).setDepth(12);
    txt.setAlpha(0.85);
    const fadeTween = this.tweens.add({ targets: txt, alpha: 0, duration: this.markerDurationTurns * 800, onComplete: () => { try { txt.destroy(); } catch (e) {} } });
    this.markers.push({ r, c, symbol, color, turnPlaced: this.turnNumber, display: txt, tween: fadeTween });

    const key = `${r}_${c}`;
    if (!this.tileMeta[key]) this.tileMeta[key] = { type: 'normal', linked: null, markers: [] };
  ...
  decayMarkers() {
    ...
    for (const m of this.markers) {
      if (this.turnNumber - m.turnPlaced >= this.markerDurationTurns) {
        try { if (m.display && m.display.destroy) m.display.destroy(); } catch (e) {}
        try { if (m.tween && m.tween.remove) m.tween.remove(); } catch (e) {}
      } else keep.push(m);
    }
    this.markers = keep;
  ...
  generatePatternFromProbs(probs = [], len = this.patternLen) {
  ...
  checkPattern(col, coherent) {
    if (!this.targetPattern) return false;
  ...
  updatePatternUI() {
    for (let i = 0; i < this.patternLen; i++) {
      ...
      const label = this.patternLabels[i];
  ...
  endTurn() {
    if (this._ending) return;
    if (this.coherence <= 0) { this.endGame(false); return; }
    this.log(`End of turn ${this.turnNumber}. Environment evolves...`);
    this.turnNumber++;
    const prevEnergy = this.energy;
    this.energy = Math.min(4, this.energy + 1);
    if (this.energy > prevEnergy) { this.log("⚡ Energy cell recharged by 1."); this.playTone(480, 0.08, 0.04); }
  ...
  visualizeProbs(probs = []) {
    if (!this._probLabelGroup) this._probLabelGroup = [];
    this._probLabelGroup.forEach(t => { try { t.destroy(); } catch (e) {} });
    this._probLabelGroup = [];
    ...
    for (let i = 0; i < (probs || []).length; i++) {
      ...
      const w = Math.max(0, (this.panelW - 44) * p);
    ...
    for (let c = 0; c < this.gridW; c++) {
      ...
      const tile = this.tiles?.[r]?.[c];
      if (tile) tile.setFillStyle(color);
    }
  }
  ...
  recordAction(key, info) {
    ...
    for (let i = 0; i < this.recentActionTexts.length; i++) {
      ...
      const txt = this.recentActionTexts[i];
      if (!act) { txt.setText(""); continue; }
      let color = "#ddd";
      ...
      txt.setColor(color);
      txt.setText(`${act.time.split(" ")[0]} ${act.key}: ${act.info}`);
    }
  }
  ...
  update(time, delta) {
    ...
    if (this.energy > 0) {
      ...
      if (Phaser.Input.Keyboard.JustDown(this.keyH) || Phaser.Input.Keyboard.JustDown(this.keyQ) || Phaser.Input.Keyboard.JustDown(this.keyE)) {
  ...
  flashEnergyWarning() {
    const warn = this.add.text(this.boardX + this.boardW / 2, 40, "⚡ OUT OF ENERGY ⚡", { fontSize: "22px", color: "#ff6688", fontStyle: "bold" }).setOrigin(0.5).setDepth(200);
    this.tweens.add({ targets: warn, alpha: { from: 1, to: 0 }, y: 10, duration: 1000, onComplete: () => warn.destroy() });
    this.cameras.main.flash(150, 255, 40, 60);
    this.playTone(160, 0.1, 0.15);
  }

  showFloatingText(text, x, y, color = "#fff") {
    const t = this.add.text(x, y, text, { fontSize: "20px", color, fontStyle: "bold" }).setOrigin(0.5).setDepth(400);
    this.tweens.add({ targets: t, y: y - 40, alpha: 0, duration: 800, ease: "Cubic.easeOut", onComplete: () => t.destroy() });
  }
  ...
  restart() {
    ...
    try {
      ...
      if (this.textures.exists('bg')) this.textures.remove('bg');
      if (this.textures.exists('scan')) this.textures.remove('scan');
      try { this.input.keyboard.removeAllKeys(); } catch (e) {}
      try { this.input.removeAllListeners(); } catch (e) {}
    } catch (e) {}

    this.cameras.main.fadeOut(300, 0, 0, 0);
    this.cameras.main.once('camerafadeoutcomplete', () => {
      if (this.scene.isActive("FinalScene")) this.scene.stop("FinalScene");
      this._restarting = false;
  ...
  endGame(win) {
    ...
    const overlay = this.add.rectangle(this.boardX + this.boardW / 2, this.canvasH / 2, this.boardW, this.canvasH, 0x000000, 0.85).setDepth(300);

    this.add.text(this.boardX + this.boardW / 2, this.canvasH / 2 - 30, msg, { fontSize: "28px", color: "#fff", fontStyle: "bold" }).setOrigin(0.5).setDepth(301);

    for (let i = 0; i < 60; i++) {
      const px = this.add.circle(this.boardX + Phaser.Math.Between(0, this.boardW), Phaser.Math.Between(0, this.canvasH), Phaser.Math.Between(1, 3), win ? 0x66ffcc : 0xff6666, 0.8).setDepth(302);
      this.tweens.add({ targets: px, alpha: 0, duration: 800 + Math.random() * 600, onComplete: () => px.destroy() });
    }

    const btn = this.add.text(this.boardX + this.boardW / 2, this.canvasH / 2 + 40, "↻ Restart Quantum Field", { fontSize: "20px", color: "#000", backgroundColor: "#9ff", padding: { x: 14, y: 8 } }).setOrigin(0.5).setDepth(305).setInteractive({ useHandCursor: true });
    btn.on("pointerover", () => btn.setStyle({ backgroundColor: "#bff" })); btn.on("pointerout", () => btn.setStyle({ backgroundColor: "#9ff" }));
    ...
    btn.once("pointerdown", restartGame);

    const menuBtn = this.add.text(this.boardX + this.boardW / 2, this.canvasH / 2 + 100, "🏠 Return to Main Menu", { fontSize: "18px", color: "#000", backgroundColor: "#9ff", padding: { x: 12, y: 7 } }).setOrigin(0.5).setDepth(305).setInteractive({ useHandCursor: true });
    menuBtn.on("pointerover", () => menuBtn.setStyle({ backgroundColor: "#bff" })); menuBtn.on("pointerout", () => menuBtn.setStyle({ backgroundColor: "#9ff" }));
    ...
    this.tweens.add({ targets: overlay, alpha: { from: 0.85, to: 0.92 }, duration: 1000, yoyo: true, repeat: -1, ease: "Sine.easeInOut" });
  }

  shutdown() {
    try {
      if (this._onPointer) { this.input.off('pointerdown', this._onPointer); this._onPointer = null; }
      try { if (this.keyH) this.input.keyboard.removeKey(this.keyH); } catch(e) {}
      try { if (this.keyQ) this.input.keyboard.removeKey(this.keyQ); } catch(e) {}
      try { if (this.keyE) this.input.keyboard.removeKey(this.keyE); } catch(e) {}
      try { if (this.keyT) this.input.keyboard.removeKey(this.keyT); } catch(e) {}
      try { if (this.keyL) this.input.keyboard.removeKey(this.keyL); } catch(e) {}
      try { if (this.spaceKey) this.input.keyboard.removeKey(this.spaceKey); } catch(e) {}
      this.input.keyboard.off('keydown-R'); this.input.keyboard.off('keydown-L');

      if (this._logTimer) { clearTimeout(this._logTimer); this._logTimer = null; }

      if (this._probLabelGroup) { this._probLabelGroup.forEach(l => { try { l.destroy(); } catch (e) {} }); this._probLabelGroup = null; }
      if (this._labelGroup) { this._labelGroup.forEach(l => { try { l.destroy(); } catch (e) {} }); this._labelGroup = null; }

      if (this.markers) {
        for (const m of this.markers) { try { if (m.display && m.display.destroy) m.display.destroy(); } catch (e) {} }
        this.markers = [];
      }

      try { if (this.textures.exists('wave')) this.textures.remove('wave'); } catch (e) {}
      try { if (this.textures.exists('bg')) this.textures.remove('bg'); } catch (e) {}
      try { if (this.textures.exists('scan')) this.textures.remove('scan'); } catch (e) {}
      ...
      try { this.input.keyboard.removeAllKeys(); } catch (e) {}
      try { this.input.removeAllListeners(); } catch (e) {}
      try { this.events.removeAllListeners(); } catch (e) {}
